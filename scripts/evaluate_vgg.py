...
def evaluate_graph(graph_file_name, photos_dir='tf_files/flower_photos'):
    with load_graph(graph_file_name).as_default() as graph:
        ground_truth_input = tf.placeholder(
            tf.float32, [None, 25], name='GroundTruthInput')
        ...
        logits = graph.get_tensor_by_name("final_training_ops/BiasAdd:0")
        xent = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            labels = ground_truth_input,
            logits = logits))

    image_dir = photos_dir
    ...
    with tf.Session(graph=graph) as sess:
        for filename, ground_truth in zip(filenames, ground_truths):
            ...
            if i % 10 == 0:\
                sys.stdout.write('\r%d %f %%' % (i, s / i * 100))
